...
FAILURE_THRESHOLD_3 = 3
FAILURE_THRESHOLD_2 = 2
FAILURE_THRESHOLD_1 = 1
FAILURE_COUNT_3 = 3
FAILURE_COUNT_2 = 2
FAILURE_COUNT_1 = 1
FAILURE_COUNT_0 = 0
RECOVERY_TIMEOUT_60 = 60
...
class TestCircuitBreakerClosedToOpen:
    ...
    async def test_closed_state_allows_calls(self) -> None:
        ...
        cb = CircuitBreaker(failure_threshold=FAILURE_THRESHOLD_3, recovery_timeout=RECOVERY_TIMEOUT_60)
    ...
    async def test_closed_to_open_after_failure_threshold(self) -> None:
        ...
        cb = CircuitBreaker(
            failure_threshold=FAILURE_THRESHOLD_3,
            recovery_timeout=RECOVERY_TIMEOUT_60,
...
class TestCircuitBreakerOpenRecovery:
    ...
    async def test_open_transitions_to_half_open_after_timeout(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        cb = CircuitBreaker(
            failure_threshold=FAILURE_THRESHOLD_1,
            recovery_timeout=RECOVERY_TIMEOUT_30,
            expected_exceptions=(ValueError,),
        )
...
class TestCircuitBreakerHalfOpen:
    ...
    async def test_half_open_success_resets_to_closed(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with pytest.raises(ValueError, match="failure"):
            await cb.call(mock_func)
        assert cb.state == CircuitState.OPEN
        assert cb.failure_count == FAILURE_COUNT_1
        ...
        assert result == "success"
        assert cb.state == CircuitState.CLOSED
        assert cb.failure_count == FAILURE_COUNT_0
...
class TestCircuitBreakerUnexpectedExceptions:
    ...
    async def test_multiple_unexpected_exceptions(self) -> None:
        ...
        cb = CircuitBreaker(
            failure_threshold=FAILURE_THRESHOLD_2,
            recovery_timeout=RECOVERY_TIMEOUT_60,
            expected_exceptions=(ValueError,),
        )

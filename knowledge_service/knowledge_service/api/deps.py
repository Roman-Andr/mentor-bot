    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            async def call_auth_service() -> httpx.Response:
                logger.debug("Calling auth service for current user")
            ...
            if response.status_code != status.HTTP_200_OK:
                logger.warning("Authentication failed: auth service returned status={}", response.status_code)
            ...
            if "Circuit breaker is OPEN" in str(e):
                logger.error("Auth service circuit breaker open")
...
async def require_hr(
    ...
    if not current_user.has_role(["HR", "ADMIN"]):
        logger.warning("HR access denied (user_id={}, role={})", current_user.id, current_user.role)
...
async def require_mentor_or_above(
    ...
    if not current_user.has_role(["MENTOR", "HR", "ADMIN"]):
        logger.warning("Mentor access denied (user_id={}, role={})", current_user.id, current_user.role)

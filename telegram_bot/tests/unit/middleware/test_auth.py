...
class TestAuthMiddleware:
    ...
    async def test_cached_user_with_refresh_token_success(self, middleware, mock_handler, mock_tg_user):
        """Test token refresh with cached user (lines 80-85)."""
        mock_message = MagicMock(spec=Message)
        mock_message.from_user = mock_tg_user
        ...
        with patch.object(auth_module.user_cache, "get_user", AsyncMock(return_value=cached_user)):
            with patch.object(auth_module.auth_client, "refresh_token", AsyncMock(return_value=refresh_result)):
                with patch.object(auth_module.user_cache, "set_user", mock_set_user):
                    result = await middleware.__call__(mock_handler, event, data)

        assert result == "handler_result"
        assert data["user"]["access_token"] == "new_token"
        assert data["user"]["refresh_token"] == "new_refresh"
        mock_set_user.assert_called_once()

    async def test_cached_user_refresh_token_failed_reauth(self, middleware, mock_handler, mock_tg_user):
        """Test token refresh fails, re-authenticates (lines 86-103)."""
        mock_message = MagicMock(spec=Message)
        mock_message.from_user = mock_tg_user
        ...
        mock_set_user = AsyncMock()
        with patch.object(auth_module.user_cache, "get_user", AsyncMock(return_value=cached_user)):
            with patch.object(auth_module.auth_client, "refresh_token", AsyncMock(return_value=None)):
                with patch.object(auth_module.settings, "TELEGRAM_API_KEY", "test_key"):
                    with patch.object(
                        auth_module.auth_client, "authenticate_with_telegram", AsyncMock(return_value=auth_result)
                    ):
                        with patch.object(auth_module.auth_client, "get_current_user", AsyncMock(return_value=user_data)):
        ...
        assert result == "handler_result"
        assert data["user"]["access_token"] == "new_access"
        mock_set_user.assert_called_once()

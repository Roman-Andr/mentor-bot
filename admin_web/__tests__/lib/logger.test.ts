...
import { pad, formatTimestamp, getCallerLocation, LoggerClass } from "@/shared/lib/logger";
...
describe("logger helper functions", () => {
  ...
  describe("getCallerLocation", () => {
    ...
    it("returns fallback when stack is unavailable", () => {
      const OriginalError = Error;
      vi.stubGlobal(
        "Error",
        class extends OriginalError {
          stack = undefined;
        },
      );

      expect(getCallerLocation()).toBe("?:?:?");

      vi.unstubAllGlobals();
    });

    it("skips blank, logger, and unparsable stack lines", () => {
      const OriginalError = Error;
      vi.stubGlobal(
        "Error",
        class extends OriginalError {

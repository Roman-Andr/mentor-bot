...
import { articlesApi, categoriesApi, attachmentsApi } from "@/shared/lib/api/articles";

const mockFetchApi = vi.fn((..._args: unknown[]) => undefined);
const mockFetchUpload = vi.fn((..._args: unknown[]) => undefined);
const mockBuildQueryString = vi.fn((..._args: unknown[]) => "");

vi.mock("@/shared/lib/api/client", () => ({
  fetchApi: (...args: unknown[]) => mockFetchApi(...args),
  fetchUpload: (...args: unknown[]) => mockFetchUpload(...args),
}));
...
describe("articlesApi", () => {
  ...
  it("list with params", () => {
    mockBuildQueryString.mockReturnValueOnce("status=published&limit=10");
    articlesApi.list({ status: "published", category_id: 5, limit: 10 });
    expect(mockFetchApi).toHaveBeenCalledWith("/api/v1/articles?status=published&limit=10");
  });

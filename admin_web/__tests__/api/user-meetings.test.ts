...
import { userMeetingsApi } from "@/shared/lib/api/user-meetings";
...
describe("userMeetingsApi", () => {
  ...
  it("list with params", () => {
    userMeetingsApi.list({ status: "scheduled", skip: 0, limit: 10 });
    expect(mockFetchApi).toHaveBeenCalledWith(
      "/api/v1/user-meetings/my?status=scheduled&skip=0&limit=10",
      undefined,
    );
  });
  ...
  it("listByMeeting with params", () => {
    userMeetingsApi.listByMeeting(123, { status: "completed", skip: 0, limit: 10 });
    expect(mockFetchApi).toHaveBeenCalledWith(
      "/api/v1/user-meetings/by-meeting/123?status=completed&skip=0&limit=10",
      undefined,
    );
  });
  ...
  it("autoAssign with params", () => {
    userMeetingsApi.autoAssign(123, { department_id: 5, position: "manager", level: "senior" });
    expect(mockFetchApi).toHaveBeenCalledWith(
      "/api/v1/user-meetings/auto-assign/123?department_id=5&position=manager&level=senior",
      { method: "POST" },
    );
  });

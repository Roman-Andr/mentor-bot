...
describe("useFocusTrap", () => {
  ...
  it("wraps focus forward on Tab from the last element and from outside", () => {
    ...
    first.focus();
    const containedEvent = new KeyboardEvent("keydown", {
      key: "Tab",
      bubbles: true,
      cancelable: true,
    });
  ...
  it("ignores non-Tab keys and handles empty focusable lists", () => {
    const { unmount } = render(
      createElement(FocusTrapFixture, null, createElement("button", { id: "first" }, "First")),
    );
    ...
    const nullRefEvent = new KeyboardEvent("keydown", {
      key: "Tab",
      bubbles: true,
      cancelable: true,
    });
    ...
    render(
      createElement(FocusTrapFixture, null, createElement("span", null, "No focusable elements")),
    );

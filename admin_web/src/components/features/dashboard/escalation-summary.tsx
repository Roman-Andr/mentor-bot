...
export function EscalationSummary({ escalations }: EscalationSummaryProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("escalations")}</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <div className="flex items-center justify-between rounded-lg bg-red-50 p-3">
            <span className="text-sm text-red-700">{t("toHr")}</span>
            <span className="font-bold text-red-700">{escalations.hr}</span>
          </div>
          <div className="flex items-center justify-between rounded-lg bg-yellow-50 p-3">
            <span className="text-sm text-yellow-700">{t("toMentor")}</span>
            <span className="font-bold text-yellow-700">{escalations.mentor}</span>
          </div>
          <div className="flex items-center justify-between rounded-lg bg-blue-50 p-3">
            <span className="text-sm text-blue-700">{t("inWork")}</span>
